"""Lesson 1.1.1 — the rogue agent with nothing in its way. The baseline everything else is measured against.
...
ELSE. But ``uv run main.py`` is still the one command you type: start the box, then run it.
    ...
    cd ../../../../infra && ./up.sh 1.1.1     # or press 'u' in the sbx-tui panel
...
RESULTS = REPO_ROOT / "results" / "1.1.1.json"
...
def main() -> None:
    ...
    banner("Part 4 — What is still open (for lesson 1.1.1, that is everything)")
    reached = card.reached()
    print(f"  {len(reached)} of the attacks landed with nothing to stop them:")
    for f in reached:
        print(f"    {f['name']:<20} {f['value']}")
    print("\n  Lesson 1.1.1 has no Part 3 — there is no rung below it to compare against. Every row above")
    print("  is a promise the rest of the tutorial keeps: lesson 1.2.1 (a container) closes most of them")
    ...
    print(f"\n  scorecard written to {RESULTS.relative_to(REPO_ROOT)} — lesson 1.2.1 reads it for its Part 3.")
