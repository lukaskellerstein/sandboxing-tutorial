...
```bash
cd tutorial/phase1-attacks/chapter-1-no-sandbox/lesson-01-no-sandbox
...
> 1.1.1`, or press `u` in the sbx-tui panel), then run `uv run python -u
...
- `bpf` is `EPERM` and `perf_event_open` is `EACCES` **because Ubuntu hardened
  ...
  lesson 1.2.3 can *open* these rows again by handing the workload a guest kernel that
  never had those settings.
- `kallsyms_readable` is false for the same reason: `kptr_restrict`.

The scorecard is written to `results/1.1.1.json`.

## Next

[`lesson-01-container`](../../chapter-2-one-host/lesson-01-container/) puts the identical suite behind a
