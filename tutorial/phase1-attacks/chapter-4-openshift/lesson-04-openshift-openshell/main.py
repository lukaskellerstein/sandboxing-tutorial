"""Lesson 1.4.4 — policy and audit on OpenShift, where the policy engine must itself pass admission.

Lesson 1.4.3 closed the kernel column with a per-pod VM and left everything else exactly where lesson
1.4.1 had it: a VM does not read HTTP, does not know which binary opened a socket, and writes nothing
down. This rung closes those, with the network still on — the same thing lesson 1.3.4 did on k3s.

What makes it a *different* lesson from lesson 1.3.4 is the collision with lesson 1.4.2. OpenShell's
...
boundary was something you switched on; this one you have to be *permitted* to switch on.
    ...
    cd tutorial/phase1-attacks/chapter-4-openshift/lesson-04-openshift-openshell && ./run.sh
"""
...
RESULTS = REPO_ROOT / "results" / "1.4.4.json"
NS = "sbx-1-4-4"
GW_NS = "openshell"
...
def main() -> None:
    ...
    try:
        ...
        print("\n  The kernel rows are the node's: this runs on ordinary runc, so lesson 1.4.3's column")
        ...
        print("  tempting, and what lesson 1.4.6 measures on this cluster's Kata (lesson 1.3.5 on gVisor).")
