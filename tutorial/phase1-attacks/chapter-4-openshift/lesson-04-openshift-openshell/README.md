# Lesson 1.4.4 — OpenShell on OpenShift

The last rung of the ladder, and the only one where the boundary has to ask permission
before it can exist.

Lesson 1.4.3 closed the kernel column with a per-pod VM and left everything else exactly where
lesson 1.4.1 had it: a VM does not read HTTP, does not know which binary opened a socket, and
writes nothing down. This rung closes those three, with the network **on** — the same thing
[`lesson-1.3.4`](../../chapter-3-kubernetes/lesson-04-k8s-openshell/) did on k3s.

What makes it a different lesson from lesson 1.3.4 is the collision with
[`lesson-1.4.2`](../lesson-02-openshift-scc/). OpenShell's supervisor builds a nested network
...
Chapter 4 shares one cluster across lessons 1.4.1–1.4.4, so unlike chapters 1–3 **`run.sh` does
not provision or destroy anything**:

```bash
../../../../infra/openshift-sno/install.sh     # ~1.5-2 h, EUR 0.263/hr. ONCE, for lessons 1.4.1-1.4.4
cd tutorial/phase1-attacks/chapter-4-openshift/lesson-04-openshift-openshell
./run.sh
...
[`policy.yaml`](policy.yaml) is deliberately almost identical to lesson 1.3.4's, and that
...
```yaml
- host: sbx-gateway.sbx-1-4-4.svc.cluster.local
  port: 8080
...
The one thing that had to change from lesson 1.3.4 is the absence of a `process:` block. Lessons
1.2.4 and 1.3.4 pin `run_as_user: sandbox` because they run this repo's own agent image. There is no
...
processes and took 512 MB. If you read lesson 1.3.4 as "OpenShell caps resources", this is the
...
is why lesson 1.3.4 reports the same `n/a`. Worth stating plainly, because "the probe returned
...
The kernel rows are the node's. This runs on ordinary runc, so **lesson 1.4.3's column is wide
...
Which is what makes stacking them tempting — and what lesson 1.3.5 measures on gVisor,
and lesson 1.4.6 on this same cluster's Kata.
