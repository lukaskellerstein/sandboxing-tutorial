# Lesson 1.4.1 — the same agent, the same pod, on OpenShift
...
cd tutorial/phase1-attacks/chapter-4-openshift/lesson-01-openshift-pod
./run.sh
```
...
There is **no `runAsUser`** in this pod spec. Lesson 1.3.1 pinned uid 1000; on OpenShift that
...
[`lesson-02-openshift-scc`](../lesson-02-openshift-scc/) is entirely about why.
...
- **`plant_backdoor` reads SUCCEEDED**, unlike lesson 1.3.1. `$HOME` is `/tmp` here — the
  ...
  Lesson 1.3.1 kept `$HOME` on the read-only rootfs and scored BLOCKED.
...
where lesson 1.3.1 blocked them. That is deliberate: this lesson is the OpenShift baseline,
and stacking a scoped egress policy on top would confuse "what OpenShift gives you by
default" with "what you can configure". Lesson 1.3.1 already measured the NetworkPolicy story.
...
- admission → [`lesson-02-openshift-scc`](../lesson-02-openshift-scc/)
- the kernel column → [`lesson-03-openshift-kata`](../lesson-03-openshift-kata/)
