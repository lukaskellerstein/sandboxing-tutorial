...
`automountServiceAccountToken: false`. The surprise is what happens to it in lesson 1.4.2.
...
RESULTS = REPO_ROOT / "results" / "1.4.1.json"
NS = "sbx-1-4-1"
...
def agent_pod(node_kernel: str) -> dict[str, object]:
    ...
    There is no `runAsUser` here. Lesson 1.3.1 pinned uid 1000; on OpenShift that is exactly what gets a
    manifest refused, because the project has its own UID range and admission assigns from it. The
    omission is the OpenShift-correct spelling of the same intent, and lesson 1.4.2 is about why.
    """
    env = {
        "PROBE_GROUPS": GROUPS,
...
def main() -> None:
    ...
    print("\n  Note what is missing from the spec: runAsUser. Lesson 1.3.1 pinned uid 1000; here the")
    print("  project assigns one. That omission is the whole of lesson 1.4.2.")
    ...
    try:
        ...
        print("  which is lesson 1.4.2 — and the kernel column waits for lesson 1.4.3.")
