"""Lesson 1.4.6 — Composition: OpenShell over Kata, on OpenShift. The commercially-relevant proof.

Lesson 1.3.6 showed OpenShell's policy holding on Kata on k3s: a real guest kernel ships Landlock, so
the filesystem clause that vanished under gVisor (lesson 1.3.5) stays enforced. This rung runs the same
composition on the platform an enterprise actually buys it on — OpenShift's **sandboxed-containers
operator**, where Kata is the product (`RuntimeClass kata`, lesson 1.4.3) and OpenShell's policy engine
must itself pass **SCC admission** (the collision lesson 1.4.2 sets up and lesson 1.4.4 pays).
...
- OpenShell's sandbox service account holds the **privileged SCC**, or its supervisor cannot build
  the nested network namespace and every sandbox dies at admission (lesson 1.4.4's preflight).

The expected reading is lesson 1.3.6's: `fs_policy_write` **blocked**, because Landlock is present in
...
node and a kernel-difference test returns a false "no VM" (Trap #12; lesson 1.4.3; REPRODUCE.md).

    ../../../../infra/openshift-sno/install.sh     bring the cluster up — ONCE, shared by lessons 1.4.1-1.4.4,1.4.6
    cd tutorial/phase1-attacks/chapter-4-openshift/lesson-06-compose-kata-openshell && ./run.sh
...
RESULTS = REPO_ROOT / "results" / "1.4.6.json"
NS = "sbx-1-4-6"
GW_NS = "openshell"
...
#: called `kata` (lesson 1.4.3) — not k3s's `kata-qemu`. Selected per sandbox via the driver-config overlay.
RUNTIME_CLASS = "kata"
...
#: A gateway accepts a SINGLE compute driver; this cannot share lesson 1.2.4's podman config.
...
def preflight() -> str:
    ...
    # The privileged SCC grant is lesson 1.4.2 reaching into this lesson, exactly as it does lesson 1.4.4:
    ...
    if "openshell-sandbox" not in subjects:
        sys.exit(
            ...
            "  die at admission. This is lesson 1.4.2 acting on lesson 1.4.6:\n"
...
def probe_env(gateway: str, collector: str) -> list[str]:
    ...
    scoping — see lesson 1.4.4). PROBE_READONLY_PATH names the path `fs_policy_write` writes to."""
    pairs = {
        "PYTHONPATH": "/tmp",
...
def create_sandbox(
    ...
    exit (lesson 1.4.4). The runtime is chosen with `--driver-config-json`; the policy is applied at
...
def wait_ready(name: str, timeout_s: int = 480) -> None:
    """`sandbox create` returns before the supervisor accepts work, and an exec then HANGS (lesson 1.4.4).
...
def deliver_suite(pod: str) -> bool:
    """Copy `infra/images/agent/attacks/` into the sandbox and prove it landed (lesson 1.4.4's routine).
...
def cleanup() -> None:
    ...
    # sandbox from this `finally` (lesson 1.4.4's note).
...
def main() -> None:
    ...
    try:
        ...
        banner("Part 3 — Deliver the suite, read the policy (the same one lesson 1.4.4 ran on runc)")
        ...
        if fs_blocked:
            ...
            print("  it did on k3s (lesson 1.3.6) and on runc (lesson 1.4.4). The operator's guest ships")
