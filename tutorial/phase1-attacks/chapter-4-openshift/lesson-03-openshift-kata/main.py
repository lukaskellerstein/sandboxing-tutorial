"""Lesson 1.4.3 — Kata as a supported product, not a DIY install.

Lesson 1.3.3 stood Kata up on k3s with `kata-deploy`'s Helm chart: a DaemonSet, a containerd drop-in, a
...
appears. The workload manifest is then byte-identical to lesson 1.3.3's. This is the deployment a large
audience will actually meet, and the interesting part is what it does NOT change.

**The reading that matters, and the reason this lesson cannot reuse lesson 1.3.3's assertion:** Red Hat
builds the Kata guest kernel from the same RHEL base as the node's, so `uname -r` inside the VM is
**identical** to the node's. Lesson 1.3.3 on k3s proves its VM by a differing kernel; that test run here
...
RESULTS = REPO_ROOT / "results" / "1.4.3.json"
NS = "sbx-1-4-3"

#: Read, never guessed — but note how much shorter the list is than lesson 1.3.3's. kata-deploy on k3s
...
def main() -> None:
    ...
    print("\n  Compare lesson 1.3.3: a Helm chart, a k8sDistribution value, a DaemonSet, and 25")
    ...
    try:
        ...
        if same_kernel:
            ...
            print("  Lesson 1.3.3 proves its VM on k3s by the kernel DIFFERING. That test, run here,")
        ...
        print("\n  And what it does not buy is the same as lesson 1.3.3: a VM per pod closes the kernel")
        print("  column and nothing else. Kata does not read HTTP, does not know which binary opened")
        print("  a socket, and writes nothing down. That is lesson 1.4.4.")
