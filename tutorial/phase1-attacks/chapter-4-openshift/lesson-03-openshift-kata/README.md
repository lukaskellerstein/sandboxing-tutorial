# Lesson 1.4.3 — Kata on OpenShift: the same boundary, as a product

Lesson 1.3.3 assembled Kata on k3s: a Helm chart, a `k8sDistribution` value that had to be
...
called exactly `kata` appears. The workload manifest is then byte-identical to lesson 1.3.3's.

```bash
cd tutorial/phase1-attacks/chapter-4-openshift/lesson-03-openshift-kata && ./run.sh
```

## The install, for contrast with lesson 1.3.3
...
## The reading that matters — and why lesson 1.3.3's test fails here

> [!warning]
> **The Kata guest kernel is byte-identical to the node's.** Red Hat builds it from the
> same RHEL base. Lesson 1.3.3 proves its VM on k3s by the kernel **differing** — run that
...
Worth carrying forward, because it is the reason lesson 1.3.3's assertion takes *either*
signal rather than both:

| | guest kernel | DMI |
| :-- | :-- | :-- |
| **k3s** (lesson 1.3.3) | **differs** (`6.18.35` vs `6.8.0`) | **absent** — minimal guest, no SMBIOS |
...
Lessons [1.2.3](../../chapter-2-one-host/lesson-03-container-kata/) and [1.3.3](../../chapter-3-kubernetes/lesson-03-k8s-kata/) both run Kata on
**two** hypervisors — QEMU and Firecracker — and lesson 1.3.3 picks between them by changing
...
So the layer lesson 1.2.3 spends a whole Part on — a hypervisor being a component *below* the
...
- [`lesson-04-openshift-openshell`](../lesson-04-openshift-openshell/) — the columns
