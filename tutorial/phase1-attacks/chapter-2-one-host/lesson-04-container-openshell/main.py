"""Lesson 1.2.4 — the attacks a container could only kill by killing the network, and the one it never could.

Lessons 1.2.2 and 1.2.3 shrank the kernel. Neither of them can answer three questions, and they are not
...
The motivating scenario is lesson 1.1.1's web injection, now made containable. A browsing agent *must*
...
which is the point a blanket on/off switch cannot express.
    ...
    cd ../../../../infra && ./up.sh 1.2.4     # or press 'u' in the sbx-tui panel
    # 2. then, as often as you like (on your machine this runs the lesson ON the box):
    cd tutorial/phase1-attacks/chapter-2-one-host/lesson-04-container-openshell && uv sync && uv run python -u main.py
...
RESULTS = REPO_ROOT / "results" / "1.2.4.json"
...
def create_sandbox(policy: Path | None) -> None:
    ...
    *"process policy cannot be changed on a live sandbox (applied at startup)"*. Only
    ...
    argv = [
        ...
        # exactly which binary attempted the SSRF that lesson 1.1.1 landed.
        *METADATA_ENV,
    ]  # fmt: skip
    if policy:
...
def main() -> None:
    ...
    try:
        ...
        print("  what lesson 1.2.1 had. Reporting only the denials would let a completely broken sandbox")
        ...
        print("  lesson 1.2.1 — no user-space kernel, no guest, nothing between the workload and Linux.")
        print("  One row does differ from lesson 1.2.1, and it is worth not glossing over: this sandbox")
        print("  exposes no /sys/module at all, so `sys_module_count` reads 0 where lesson 1.2.1 read 179.")
        ...
        print("  them tempting, and what lesson 1.3.5 shows going wrong: on gVisor, Landlock returns")
