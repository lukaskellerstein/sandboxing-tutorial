# Lesson 1.2.2 — Container + gVisor

The shortest change in the tutorial and the biggest one to the kernel rows: the
same image, the same nine attacks, the same container flags as lesson 1.2.1, and one
...
- **It is not a VM.** There is no guest kernel image and no virtual hardware —
  which is exactly why lesson 1.2.3 exists, because a *real* guest kernel keeps
...
against lesson 1.2.1's recorded file would compare two machines as well as two
...
nothing on the network axis.** Lesson 1.2.3 pays far more for kernel isolation — a
whole VM — and gets exactly the same four rows left open. Only lesson 1.2.4 closes
...
[Lesson 1.2.3](../lesson-03-container-kata/) buys the same result with a **real** guest
kernel in a per-container VM, and the difference between the two only shows up in the
composition leaves: gVisor answers `ENOSYS` to Landlock (lesson 1.3.5), Kata does not
(lesson 1.3.6).

The rows gVisor leaves open are not weaker versions of attack 8 — they are a
different axis, and [lesson 1.2.4](../lesson-04-container-openshell/) is where they
