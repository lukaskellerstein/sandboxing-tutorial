"""Lesson 1.2.3 — the same kernel result as gVisor, by a completely different route: a real guest VM.
...
much. Which is a *runtime* and which is a *hypervisor* is ``docs/isolation-layers.md``, not here.
    ...
    cd ../../../../infra && ./up.sh 1.2.3     # or press 'u' in the sbx-tui panel
    # 2. then, as often as you like (on your machine this runs the lesson ON the box):
    cd tutorial/phase1-attacks/chapter-2-one-host/lesson-03-container-kata && uv sync && uv run python -u main.py
"""
...
RESULTS = REPO_ROOT / "results" / "1.2.3.json"
...
# Identical to lessons 1.2.1 and 1.2.2, minus podman's spelling. nerdctl takes the same flags, which is
...
def preflight() -> list[str]:
    ...
    if platform.system() != "Linux":
        sys.exit(
            ...
            "  Run this lesson on its box:  cd infra && ./up.sh 1.2.3\n"
            "                               ./run.sh 1.2.3"
        )
...
def merge_sandbox_death(card: Card, rc: int, runtime: str) -> Card:
    """Same host-side rescue as lesson 1.2.2 — see its docstring. A dead box still reported a verdict."""
...
def startup_seconds(nerdctl: list[str], runtime: str | None, reps: int = 3) -> float:
    ...
    Min rather than mean, for the reason lesson 1.3.3's ``time_pod_startup`` gives: one sample on a
    ...
    This is the shortest path either hypervisor has — `nerdctl run` and nothing else. Lesson 1.3.3 times
...
def main() -> None:
    ...
    print("\n  Same flags as lessons 1.2.1 and 1.2.2:")
    ...
    print("  anything down. That is lesson 1.2.4.")
    print("\n  The difference that only matters in lesson 1.3.6: this is a REAL kernel, so it ships")
