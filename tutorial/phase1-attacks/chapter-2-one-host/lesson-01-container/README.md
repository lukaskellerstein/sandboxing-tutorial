# Lesson 1.2.1 — Container

The single biggest jump in the tutorial. The same image and the same nine attacks
as lesson 1.1.1, now behind a hardened rootless container. Several attacks die here;
the groups that survive are the whole rest of the tutorial.

```bash
cd tutorial/phase1-attacks/chapter-2-one-host/lesson-01-container
...
Run lesson 1.1.1 first (`../../chapter-1-no-sandbox/lesson-01-no-sandbox`) so this lesson's Part 3 has a
...
gVisor (lesson 1.2.2) nor Kata (lesson 1.2.3) helps — neither reads HTTP — which is why
lesson 1.2.4 exists.

## Part 3 re-runs the no-sandbox rung live

On its own disposable box this lesson does not read lesson 1.1.1's recorded file — it
...
a sandbox that cannot run an agent, which is not a fix. Lessons 1.2.2 and 1.2.3 leave them
exactly where they are; only lesson 1.2.4 closes them with the network still on.
...
- `bpf` and `perf_event_open` were **already** refused in lesson 1.1.1 — Ubuntu's
...
- [`lesson-02-container-gvisor`](../lesson-02-container-gvisor/) swaps one word —
  `--runtime runsc` — and watches the kernel rows collapse. The network rows do
  not move an inch.
- [`lesson-04-container-openshell`](../lesson-04-container-openshell/) is the first
  ...
  That is [`lesson-04-container-openshell`](../lesson-04-container-openshell/).
