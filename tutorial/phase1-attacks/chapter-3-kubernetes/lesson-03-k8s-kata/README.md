# Lesson 1.3.3 — Kubernetes with Kata Containers

The same kernel result as lesson 1.3.2, by a completely different route: a **real Linux
kernel, in its own VM, per pod**. The workload manifest differs from lesson 1.3.1's by one
line — exactly as lesson 1.3.2's did — but everything underneath it is different, and the
scorecard shows where that matters.

```bash
cd tutorial/phase1-attacks/chapter-3-kubernetes/lesson-03-k8s-kata
...
Lesson 1.2.3 had to stand up a **whole second container stack** — containerd + nerdctl
...
plain pod refused them — lesson 1.2.3 measured exactly this reversal.
...
syllabus says to read the matrix and never the count: this rung and lesson 1.3.1 both
...
different people for different threats. Lesson 1.2.3 measured the same reversal on a host.
...
stock kernel without the node's mitigations. Compare lesson 1.3.2's 2.51×: same kernel
...
**Lesson 1.2.3 taught the mechanism on a host; this rung teaches the selection.** There it
...
lesson 1.3.1 — **cannot tell these two apart**. The PCI bus can: Firecracker boots `pci=off`
...
lesson 1.2.3, through `nerdctl run`, Firecracker started ~0.4 s ahead of QEMU every time.
...
to see the hypervisor's own start cost, lesson 1.2.3 is where it is visible.
...
The same rows lesson 1.3.2 left open, for the same reason. A VM per pod buys attack 8 and
...
Landlock — which gVisor's user-space kernel answers `ENOSYS` to. Lesson 1.3.6 stacks a
policy engine on this same Kata runtime and the composition *holds*; lesson 1.3.5 stacks
