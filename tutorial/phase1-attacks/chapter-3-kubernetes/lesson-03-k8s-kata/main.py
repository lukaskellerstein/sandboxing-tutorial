"""Lesson 1.3.3 — the same kernel result as gVisor, by a completely different route.

Lesson 1.3.2 changed one field and got a **user-space** kernel. This lesson changes the same field and
gets a **real Linux kernel, in its own VM, per pod**. The workload manifest differs from lesson 1.3.1's
by one line, exactly as lesson 1.3.2's did — but everything under it is different, and the scorecard
shows where that matters.

This is also where chapter 2's awkwardness pays off. Lesson 1.2.3 had to stand up a whole second
...
* Kata's guest kernel is **less hardened than the node's**, so ``bpf()`` and ``io_uring_setup``
  *reopen* here after a plain pod refused them. Lesson 1.2.3 measured the same reversal. A stronger
...
**hypervisor** under the runtime. Lesson 1.2.3 taught that mechanism on a host, where it took a shim
...
working.
    ...
    cd ../../../../infra && ./up.sh 1.3.3     # or press 'u' in the sbx-tui panel
    # 2. then, as often as you like (on your machine this runs the lesson ON the box):
    cd tutorial/phase1-attacks/chapter-3-kubernetes/lesson-03-k8s-kata && uv sync && uv run python -u main.py
"""
...
NAMESPACE = "sbx-1-3-3"
RESULTS = REPO_ROOT / "results" / "1.3.3.json"
...
def merge_pod_death(card: Card, reason: str, label: str) -> Card:
    """Fill in the row a dead pod could not report — see lesson 1.3.1, which meets this first.
...
def report_hypervisors(card: Card, gateway_ip: str, qemu_class: str, fc_class: str) -> dict[str, object]:
    ...
    Lesson 1.2.3 taught the *mechanism*: a hypervisor is a component below the runtime, selected by the
    ...
    **no row** of the security matrix. Both sit on KVM and hand the workload the same guest kernel.
    """
    print("  The whole of lesson 1.2.3's Part 3b — a shim config, a snapshotter, a block device —")
    ...
    print("\n    Lesson 1.2.3 measured the same two hypervisors through `nerdctl run` and Firecracker came")
...
def main() -> None:
    ...
    try:
        ...
        print("  The manifest is lesson 1.3.1's. The entire difference is:\n")
        ...
        print("  is a stock kernel without the node's hardening. Compare gVisor's 2.5x in lesson 1.3.2:")
        ...
        print("\n  The same rows lesson 1.3.2 left open, for the same reason: a VM per pod buys attack 8")
        print("  and buys nothing on attacks 2, 4, 5, 6 or 9. Kata does not read HTTP, does not know")
        print("  which binary opened a socket, and writes nothing down. Lesson 1.3.4 does all three.")
        print("\n  One difference from lesson 1.3.2 matters later: this is a REAL kernel, so it ships")
        print("  Landlock, which gVisor's user-space kernel answers ENOSYS to. Lesson 1.3.6 stacks a")
        print("  policy engine on this same Kata runtime and the composition holds; lesson 1.3.5 stacks")
