"""Lesson 1.3.5 — Composition: OpenShell over gVisor. Where stacking makes you *less* safe.

Lesson 1.3.4 ran OpenShell's policy on ordinary runc and every clause held: the L7 egress rules, the
...
one field lesson 1.3.2 selected — and measures which clauses survive the swap.
...
exactly as in lesson 1.3.2), and the L7 policy is unaffected because OpenShell's proxy does not read the
...
fix — it refuses to start rather than run a policy it cannot fully enforce. Under Kata (lesson 1.3.6)
...
`docs/isolation-layers.md` § *The trap*.
    ...
    cd tutorial/phase1-attacks/chapter-3-kubernetes/lesson-05-compose-gvisor-openshell && uv sync && uv run python -u main.py
"""
...
NAMESPACE = "sbx-1-3-5"
RESULTS = REPO_ROOT / "results" / "1.3.5.json"
...
def preflight() -> str:
    """Confirm the CLI is talking to the in-cluster gateway, on the kubernetes driver (lesson 1.3.4's check)."""
...
def wait_ready(name: str, timeout_s: int = 300) -> None:
    """Block until the sandbox reports Ready — an exec issued before then HANGS (lesson 1.3.4's fix)."""
...
def main() -> None:
    ...
    try:
        banner("Part 1 — The same policy as lesson 1.3.4, on a gVisor sandbox instead of runc")
        ...
        # and every reading is meaningless. This is lesson 1.3.4's assertion, and it is the CONTROL that
        ...
        if n_high and fs_blocked:
            ...
            print("  (lesson 1.3.6), and the HIGH audit finding is the ONLY thing that differs. The lesson:")
        ...
        print("  (lesson 1.3.6) nothing is lost, because the guest ships Landlock. The rule holds:")
...
def box_ip_if_any() -> str | None:
    """The IP of this lesson's box, from infra's state file — or None if there is no box (lesson 1.3.4's gate)."""
    try:
...
def run_on_box(ip: str) -> int:
    """A box is up but this is not it — run the lesson ON the box via infra/run.sh (lesson 1.3.4's delegate)."""
