# Lesson 1.3.5 — Composition: OpenShell over gVisor

The tutorial has spent nine lessons showing that gVisor (lesson 1.3.2) and OpenShell
(lesson 1.3.4) are strong in **disjoint columns**: gVisor shrinks the host-kernel
...
```bash
cd tutorial/phase1-attacks/chapter-3-kubernetes/lesson-05-compose-gvisor-openshell
...
Lessons 1.3.1–1.3.4 and 1.3.5–1.3.6 all share one cluster (`chapter-03-k8s`), so the cheap way
...
## The one thing that changes from lesson 1.3.4

Lesson 1.3.4 created its sandbox on the cluster's default runtime (runc) and passed no
...
Everything else is lesson 1.3.4's: the same `policy.yaml`, the same probe suite, the
...
- **The kernel column closes.** Exactly as in lesson 1.3.2 — `bpf`, `io_uring_setup`
...
- **…but the write stays blocked, so nothing looks wrong.** `fs_policy_write` —
  the probe that writes `/etc/agent-probe-canary`, which lesson 1.3.4 refused — is
  **still refused here**. On this box every read-only path (`/etc`, `/app`,
  ...
  (lesson 1.3.6), and the audit finding is the only thing that differs.
...
Contrast lesson 1.3.6 on the same box, where the audit trail reads
...
[lesson 1.2.5](../../chapter-2-one-host/lesson-05-compose-gvisor-openshell/README.md)),
and chapter 4 cannot (gVisor is not a supported OpenShift runtime;
[lesson 1.4.5](../../chapter-4-openshift/lesson-05-compose-gvisor-openshell/README.md)).
Both document the reason and point back here.

The rule this rung establishes — and [lesson 1.3.6](../lesson-06-compose-kata-openshell/README.md)
