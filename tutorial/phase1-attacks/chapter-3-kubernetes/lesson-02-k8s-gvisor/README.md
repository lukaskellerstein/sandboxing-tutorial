# Lesson 1.3.2 — Kubernetes with gVisor

The shortest lesson in the tutorial, deliberately. Lesson 1.2.2 installed `runsc` and
...
the attack suite — is byte-identical to lesson 1.3.1. That is what makes Part 3 a
measurement of the *runtime* rather than of two different pods.

```bash
cd tutorial/phase1-attacks/chapter-3-kubernetes/lesson-02-k8s-gvisor
...
this lesson runs happily on the same 4 GB `PLAY2-NANO` as lessons 1.1.1–1.2.2 and 1.3.1. The
`ptrace` and `kvm` platforms exist; neither is the default.

That is also the cleanest way to see the difference from lesson 1.3.3: Kata needs
...
budget faster than they do on runc. Lesson 1.2.2 recorded exactly this
...
**`boundaries that held: 16/19`**, against lesson 1.3.1's 14/19.
...
The kernel column is closed. The network column is **exactly where lesson 1.3.1 left
...
- [`lesson-03-k8s-kata`](../lesson-03-k8s-kata/) reaches the same kernel result by a
  ...
  lesson 1.3.5 stacks OpenShell on gVisor (Landlock gone), lesson 1.3.6 on Kata (it holds).
- [`lesson-04-k8s-openshell`](../lesson-04-k8s-openshell/) is the rung that closes
