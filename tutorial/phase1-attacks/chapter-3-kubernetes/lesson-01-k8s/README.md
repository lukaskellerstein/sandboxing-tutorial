# Lesson 1.3.1 — Kubernetes

The same attacks, now in a hardened Pod that a cluster scheduled. Kubernetes
**composes** the primitives lesson 1.2.1 already showed and invents no new boundary —
...
```bash
cd tutorial/phase1-attacks/chapter-3-kubernetes/lesson-01-k8s
...
`RuntimeClass` field lessons 1.3.2 and 1.3.3 turn is the same field on any cluster, and it is
...
Every control below has a lesson-1.2.1 twin. What changed is that you now **declare** it,
and a cluster could refuse you — which is lesson 1.4.2's subject, on OpenShift.
...
`ephemeral-storage` has no lesson-1.2.1 equivalent. It is how a cluster caps the *disk*
...
Lesson 1.2.1 passed `--pids-limit 128` as a container flag; a Pod has no field for it.
...
Lesson 1.2.1's container could say "network" or "no network" and nothing in between. With
...
## Part 3 re-runs lesson 1.2.1's container live

Not from `results/1.2.1.json` — that card was measured on a *different machine*,
...
because the kubelet running the pod is root too. Lesson 1.2.1's own headline rung is
rootless, so this is not a re-creation of lesson 1.2.1's card — it is lesson 1.2.1's
...
The comparison against lesson 1.2.1's hardened container, re-run live on the same box
minutes earlier:
...
The rows that make lesson 1.3.4 necessary:
...
And attack 7 behaved differently from lesson 1.2.1, at the same 256Mi limit:
...
Lesson 1.2.1's container **refuses** the allocation and keeps running. This pod is killed
...
[`lesson-02-k8s-gvisor`](../lesson-02-k8s-gvisor/) changes one field and watches those
rows collapse.
...
that anything was attempted. Attack 9 is exactly as open as it was in lesson 1.1.1.

Both of those are [`lesson-04-k8s-openshell`](../lesson-04-k8s-openshell/).

## Next

- [`lesson-02-k8s-gvisor`](../lesson-02-k8s-gvisor/) — `runtimeClassName: gvisor`,
  the shortest lesson in the tutorial, and the kernel rows collapse.
- [`lesson-03-k8s-kata`](../lesson-03-k8s-kata/) — the same one-line change, reaching
  a real guest kernel in a per-pod VM.
- [`lesson-04-k8s-openshell`](../lesson-04-k8s-openshell/) — per-binary and
