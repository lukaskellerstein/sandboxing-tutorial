# Lesson 1.3.6 — Composition: OpenShell over Kata

Lesson 1.3.5 stacked OpenShell's policy on gVisor and watched the filesystem clause
...
```bash
cd tutorial/phase1-attacks/chapter-3-kubernetes/lesson-06-compose-kata-openshell
...
The same single field lesson 1.3.5 changed — OpenShell's per-sandbox driver-config
...
Services — is lesson 1.3.4's, unchanged. The runtime underneath is the only variable.
...
guest kernel matches the node, so [lesson 1.4.6](../../chapter-4-openshift/lesson-06-compose-kata-openshell/README.md)
asserts by DMI/virtio instead; here the kernel difference is the honest test.)

## What the run shows

The same three clauses as lesson 1.3.5, all three intact this time:
...
- **The filesystem clause holds too.** `fs_policy_write` stays **blocked** — the
  write to `/etc` that lesson 1.3.4 refused is refused here as well, because
  OpenShell's `landlock()` call succeeds inside the guest and the ruleset is
  actually built. The audit trail's witness is the positive mirror of lesson 1.3.5's:
...
Every scored row is identical to lesson 1.3.5's gVisor run. The two diverge **only**
