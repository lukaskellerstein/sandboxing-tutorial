# Lesson 1.3.4 — OpenShell on the cluster: policy and audit at fan-out scale

Lesson 1.3.1 closed attacks 2, 4, 5 and 6 with a NetworkPolicy, then showed the ceiling: a
`POST` to the **same allowed host** succeeded, a `curl` copied to an unnamed path
succeeded, and nothing anywhere recorded that either was attempted. Lessons 1.3.2 and 1.3.3
...
```bash
cd tutorial/phase1-attacks/chapter-3-kubernetes/lesson-04-k8s-openshell
...
## What actually changed from lesson 1.2.4

Almost nothing, and that is the point worth taking away. Lesson 1.2.4 ran the **podman**
driver on one machine. This is the **kubernetes** driver, where each sandbox is a Pod.

The policy file is lesson 1.2.4's, and the only line that had to move is the endpoint's
host — because the gateway is now a Service rather than a host alias:

```diff
-      - host: host.openshell.internal
-        port: 18410
+      - host: sbx-gateway.sbx-1-3-4.svc.cluster.local
+        port: 8080
```
...
1. **One compute driver per gateway.** A gateway accepts a single driver. Lesson 1.2.4's is
...
4. **`create` returns before the sandbox accepts work.** An `exec` issued in that
   ...
   first. Lesson 1.2.4 hit this too; the cluster driver only widens the window, because a
...
NetworkPolicy could draw that distinction too — lesson 1.3.1 did. The next two probes are
where it cannot follow:

- `http_method_denied` — a `POST` to the **same allowed host**, which lesson 1.3.1 let
...
**The three rows lesson 1.3.1 could not close**, side by side with what a NetworkPolicy
managed on the identical probes:

| probe | lesson 1.3.1 (NetworkPolicy) | lesson 1.3.4 (OpenShell) |
| :-- | :-- | :-- |
...
does on lesson 1.2.4 (`3`, SUCCEEDED): the policy grants `read_write: [/sandbox, /tmp,
...
> [!note]
> **`k8s_sa_token` reads `n/a-no-cluster` here**, unlike lessons 1.3.1–1.3.3 where it is
...
exactly as it did in lesson 1.3.1.
...
[`lesson-05-compose-gvisor-openshell`](../lesson-05-compose-gvisor-openshell/) measures
what actually happens when you do (and [`1.3.6`](../lesson-06-compose-kata-openshell/)
