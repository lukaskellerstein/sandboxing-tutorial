"""Lesson 1.3.4 — policy and audit at cluster scale.

Lesson 1.3.1 closed attacks 2, 4, 5 and 6 with a NetworkPolicy, and then showed the ceiling: a `POST` to
the same allowed host succeeded, a `curl` copied to an unnamed path succeeded, and nothing anywhere
recorded that either was attempted. Lessons 1.3.2 and 1.3.3 changed the kernel underneath and moved none of
...
Lesson 1.2.4 did the same thing with the **podman** driver, on one machine. What is worth noticing is how
little changes: the policy file is lesson 1.2.4's, and the only line that had to move is the endpoint's
...
what lesson 1.3.5 is built on, and this lesson is its clean control: nothing is stacked underneath.

    # 1. start the box (once):
    cd ../../../../infra && ./up.sh 1.3.4     # or press 'u' in the sbx-tui panel
    # 2. then, as often as you like (on your machine this runs the lesson ON the box):
    cd tutorial/phase1-attacks/chapter-3-kubernetes/lesson-04-k8s-openshell && uv sync && uv run python -u main.py
"""
...
NAMESPACE = "sbx-1-3-4"
RESULTS = REPO_ROOT / "results" / "1.3.4.json"
...
def preflight() -> str:
    ...
    A gateway accepts a **single** compute driver. Lesson 1.2.4's gateway is podman; this one is
...
def wait_ready(timeout_s: int = 300) -> None:
    ...
    Lesson 1.2.4 hit this and the fix is the same; the cluster driver only makes the window longer,
...
def refuse_no_box() -> None:
    ...
    print("the stack is k3s plus the Agent Sandbox controller and the OpenShell gateway, installed")
    print("on the box by its substrates.")
...
def main() -> None:
    ...
    try:
        ...
        banner("Part 4 — What is still open (and why lesson 1.3.5 exists)")
        ...
        print("  runs on ordinary runc, so attack 8 works exactly as it did in lesson 1.3.1. gVisor and")
        ...
        print("  — and lesson 1.3.5 measures what happens when you do.")
