...
def reason_for_failure(manifest: dict[str, object], namespace: str, *, timeout: int = 90) -> str:
    ...
    Used to show what asking for a RuntimeClass nobody registered actually looks like. The answer is
    worth seeing: the API server **accepts** the pod — there is no admission check that a
    RuntimeClass exists — and the refusal happens further down, where the sandbox is created. So the
    message has to be dug out of the pod's status or its events rather than caught as an apply error.
    """
    name = str(manifest["metadata"]["name"])  # pyright: ignore[reportIndexIssue]
    kubectl("-n", namespace, "delete", "pod", name, "--ignore-not-found", "--now", check=False)
    try:
...
def read_from_pod(namespace: str, runtime_class: str | None, command: list[str], *, name: str = "reader") -> str:
    """Run one short command inside a pod and return what it printed.

    Used for the readings that are not attacks — `uname -r`, the DMI vendor — and taken from INSIDE
    the sandbox for the reason this whole repo exists: the only trustworthy answer to "what am I
    running on" is the one the sandbox itself gives.
    """
    ...
    _, logs, _ = run_pod(manifest, namespace, timeout=300)
    return logs.strip()


def time_pod_startup(namespace: str, runtime_class: str | None, *, reps: int = 3) -> float:
    """Seconds from `apply` to a terminal phase, for a pod that does nothing. Min of ``reps``.

    This is the per-pod VM boot tax, measured rather than asserted — and it deliberately measures the
    WHOLE round trip a user waits for, not the hypervisor's boot time in isolation. The prior art's
    finding worth preserving is that the famous "Kata boots a VM per pod" cost did **not** dominate:
    scheduling, image handling and the kubelet's own loop swamped it. A lesson that asserts a tax
    without printing the number teaches the folklore instead of the measurement.

    Min-of-three rather than a mean: a single sample on a shared machine measures the scheduler's mood
    as much as the runtime, and the minimum is the closest thing to "what this costs when nothing else
    is in the way".
    """
    best = float("inf")
    for i in range(reps):
        name = f"startup-probe-{i}"
        spec: dict[str, object] = {
            "restartPolicy": "Never",
            "automountServiceAccountToken": False,
            "containers": [
