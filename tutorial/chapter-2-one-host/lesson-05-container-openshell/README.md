# Lesson 5 — Container + NVIDIA OpenShell

The survivors. Lessons 3 and 4 shrank the kernel two different ways and left the
...
And the four attacks that follow directly from those blind spots. Lessons 2, 3 and
4 each run the suite twice — once with `--network none`, once with the network an
agent actually needs — and each loses the same four the moment it is online:

| | 2 container | 3 + gVisor | 4 + Kata | **5 + OpenShell** |
| :-- | :-- | :-- | :-- | :-- |
...
```bash
cd tutorial/chapter-2-one-host/lesson-05-container-openshell
...
Lesson 1's **web injection**, now made containable. A browsing agent *must* have
...
That is not a rhetorical framing — it is on the scoreboard. Lesson 2 took the first
option, `--network none`, and scored 11/13; its Part 4 takes the second and scores
**7/13**, with attacks 2, 4, 5 and 6 all returning. Lessons 3 and 4 repeat the
...
**One driver per gateway.** A gateway accepts a single compute driver, so lesson 9's
...
lesson 2 — no user-space kernel, no guest, nothing between the workload and Linux.

One attack-8 row does *not* match lesson 2, and it is worth not glossing over: this
sandbox presents no `/sys/module` at all, so `sys_module_count` reads **0** where
lesson 2 read **179**. That is a filesystem the sandbox does not expose, not a
...
lesson 2's `ENOSYS`: same verdict, refused by a dropped capability rather than by a
seccomp filter.
...
the container hardening of lesson 2, it is a different axis layered beside it.
...
> `results/lesson-05.json`, so drift shows up as a changed number rather than a
> mysterious failure.
...
and URL — the same SSRF that landed unimpeded in lesson 1.
