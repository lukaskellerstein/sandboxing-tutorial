"""Lesson 17 — Composition: OpenShell over Kata. The stack that actually works.

Lesson 16 stacked OpenShell's policy on gVisor and watched the filesystem clause silently vanish,
because gVisor answers `ENOSYS` to `landlock()`. This rung stacks the SAME policy on Kata instead —
`runtimeClassName: kata-qemu`, the one field lesson 8 selected — and the clause holds.
...
lesson 9. Nothing regressed. Read against lesson 16, this is the positive half of the same finding:
...
buys back the Landlock the composition depends on. See `docs/isolation-layers.md` § *The trap*.
    ...
    cd tutorial/chapter-3-kubernetes/lesson-17-compose-kata-openshell && uv sync && uv run python -u main.py
"""
...
LESSON = "lesson-17-compose-kata-openshell"
STATE_ENV = REPO_ROOT / "infra" / ".state" / f"{LESSON}.env"
NAMESPACE = "sbx-lesson-17"
RESULTS = REPO_ROOT / "results" / "lesson-17.json"
...
def create_sandbox(
    ...
    Same shape as lesson 16 — the runtime is chosen with `--driver-config-json`, the policy is applied
...
def landlock_witness(name: str) -> list[str]:
    ...
    BUILT — the positive mirror of lesson 16's HIGH 'WITHOUT restrictions' finding."""
    time.sleep(4)
...
def main() -> None:
    ...
    try:
        banner("Part 1 — The same policy as lesson 9, on a Kata sandbox instead of runc")
        ...
        if fs_blocked:
            ...
            print("  9 refused is refused here too, on the SAME policy. Landlock is present in the Kata")
            print("  guest kernel, so nothing regressed. This is what lesson 16 could not do on gVisor.")
        else:
        ...
        # Passed at CREATE, never `policy set`. On gVisor (lesson 16) this refuses to start; here it
        ...
        print("  gVisor could not sustain (lesson 16). The cost is Kata's weight: a VM per pod rather")
