"""Lesson 6 — the same attacks, now in a hardened Pod that a cluster scheduled.

Kubernetes **composes** what lesson 2 already showed and invents no new boundary. Every control here
...
cluster adds is a scheduler, a declarative way to ask, and two things a single host never had:
  ...
  * **a network verdict better than on/off.** Lesson 2's container could say "network" or "no
...
on the node. See ``infra/substrates/chapter-3/60-k8s.sh``.
    ...
    cd tutorial/chapter-3-kubernetes/lesson-06-k8s && uv sync && uv run python -u main.py
"""
...
NAMESPACE = "sbx-lesson-06"
RESULTS = REPO_ROOT / "results" / "lesson-06.json"
...
# has a lesson-2 twin; what changed is that you now *declare* it and admission could refuse you.
POD_SECURITY = {
    "runAsNonRoot": True,
    "runAsUser": 1000,
    "runAsGroup": 1000,
...
# Lesson 2's flags, re-run live in Part 3 so the comparison is two boundaries and not two machines.
CONTAINER_HARDENING = [
    "--cap-drop", "ALL",
...
POLICY_SETTLE_S = 20
#: What lesson 2 ran, so Part 3's diff compares like with like.
            ...
            "containers": [
                {
                    ...
                    # so while the rung looked weaker than lesson 2 for no reason but this mount.
                    # Lesson 2 leaves $HOME on the read-only rootfs; matching it is what keeps the
...
def previous_rung(gateway_ip: str) -> tuple[Card | None, str]:
    """Lesson 2's rung — the hardened container — re-run live on this very box.

    Reading the recorded ``results/lesson-02.json`` would compare a pod here against a container
    ...
    kubelet running the pod is root too. Lesson 2's own headline rung is rootless — so this is not a
    re-creation of lesson 2's card, it is lesson 2's *boundary* at the pod's privilege level, which
...
def merge_pod_death(card: Card, reason: str) -> Card:
    ...
    Attack 7 allocates against a 256Mi limit. Lesson 2's container survives it and reports
    ...
    lesson's manifest — the cap is the same 256Mi as lesson 2's. Under cgroup v2, a container's
...
def main() -> None:
    ...
    try:
        ...
        banner("Part 3 — What just changed (lesson 2's container, re-run live on this same box)")
        ...
        if prev is None:
            ...
            print("  the boundary; it did not strengthen it. Lessons 7 and 8 are where that changes.")
        ...
        print("  one line down about what it dropped. That is lesson 9.")
        print("\n  And every kernel row is still the node's — attack 8 is as open as it was in lesson")
        print("  1. Lesson 7 changes one field and watches it collapse.")
