...
def arm(namespace: str, extra_uids: tuple[int, ...] = ()) -> str:
    ...
    # `extra_uids` exists for 2.4.4, and the need for it is a finding in itself: OpenShell owns its
    # sandbox pod, so the uid it runs as is neither the image's 1001 nor the lesson namespace's
    # assigned range. A rule scoped to the ranges this module can GUESS records nothing of it —
    # measured: 0 paths attributed to a runc sandbox whose syscalls demonstrably reach this kernel.
    # The caller reads the real uid out of the running pod and passes it here, which is why arming
    # has to happen after the sandbox is Ready and before the suite runs.
    extra = "".join(
        f"auditctl -a always,exit -F arch=b64 -S openat,open -F 'uid={u}' -k {KEY} >/dev/null 2>&1\n"
        for u in extra_uids
    )
    # AND a rule scoped by SELinux TYPE rather than uid, which is what actually catches an
    # OpenShell sandbox. Its pod spec sets no `runAsUser` — OpenShell takes the uid from its image
    # under the privileged SCC it was granted — so there is no uid for this module to guess or read,
    # and a uid-scoped rule records nothing of it (measured: 0 paths attributed to a runc sandbox
    # whose syscalls demonstrably reach this kernel).
    #
    # `subj_type=container_t` is the honest filter for "any container on this node", and it composes
    # exactly with how attribution already works here: the rule decides what the kernel records, the
    # MCS on each record decides whose it was. It records more than this pod, which is why the read
    # side spans every rotated segment.
    extra += f"auditctl -a always,exit -F arch=b64 -S openat,open -F subj_type=container_t -k {KEY} >/dev/null 2>&1\n"
...
def trail() -> tuple[str, list[str]]:
    ...
    # Oldest first, so records read in the order they were written. EVERY segment the node kept:
    # the container-wide rule above fills 8 MB fast, and capping at three was enough only while
    # the rule was uid-scoped.
    segments = [s for s in segments if s.startswith("audit.log")][:5]
