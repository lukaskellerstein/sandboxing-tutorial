...
def ensure_namespace(name: str) -> None:
    """Create the namespace, waiting out any previous one that is still terminating.

    The wait is not politeness. `delete_namespace` returns immediately and OpenShift tears a project
    down asynchronously, so a second run started straight after a first finds the namespace in
    `Terminating`: the `apply` succeeds (it is the same object) and the first pod create then fails
    with `namespaces "..." not found`. Measured back-to-back on this cluster — an audit lesson that
    cannot be run twice in a row is not a lesson anyone can trust.
    """
    for _ in range(60):
        phase = oc("get", "ns", name, "-o", "jsonpath={.status.phase}", check=False)
        if phase != "Terminating":
            break
        time.sleep(3)
    manifest = {"apiVersion": "v1", "kind": "Namespace", "metadata": {"name": name}}
    oc("apply", "-f", "-", stdin=json.dumps(manifest))
    # And wait for it to be usable: `apply` returns before the project's SCC/uid annotations land,
    # and the pod spec below depends on them.
    for _ in range(60):
        if oc("get", "ns", name, "-o", "jsonpath={.status.phase}", check=False) == "Active":
            return
        time.sleep(2)
...
def run_pod(
    manifest: dict[str, object], namespace: str, *, timeout: int = 600, container: str = "", delete: bool = True
) -> tuple[str, str, str]:
    ...
    ``container`` and ``delete`` are this leaf's additions, and 2.4.3 needs both: its pod carries a
    workload AND a sensor sidecar, so `oc logs` refuses without a container name, and the sidecar's
    own report has to be read before the pod is removed.
    """
    ...
    while time.monotonic() < deadline:
        ...
        reason = oc("-n", namespace, "get", "pod", name, "-o", f"jsonpath={sel}.state.waiting.reason}}", check=False)
    ...
    logs = oc(*logs_argv, check=False, timeout=120)
    sel = f'{{.status.containerStatuses[?(@.name=="{container}")]' if container else "{.status.containerStatuses[0]"
    term = oc("-n", namespace, "get", "pod", name, "-o", f"jsonpath={sel}.state.terminated.reason}}", check=False)
    if delete:
