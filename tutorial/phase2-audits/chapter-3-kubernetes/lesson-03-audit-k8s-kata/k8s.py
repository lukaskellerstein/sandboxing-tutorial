...
def run_pod(
    manifest: dict[str, object], namespace: str, *, timeout: int = 900, container: str = "", delete: bool = True
) -> tuple[str, str, str, str]:
    """Run one pod to completion. Returns ``(phase, logs, termination_reason, container_id)``.

    ``container`` names which container to read, and on this leaf it is required rather than a
    nicety: the pod has a workload AND a sensor, and `kubectl logs` on a multi-container pod refuses
    outright ("a container name must be specified"). Every status read below is likewise selected by
    name — `containerStatuses[0]` happens to be the right one only because `agent` sorts before
    `sensor`, which is a coincidence and not a contract.

    The fourth value is this leaf's addition to the phase-1 version, and it is what makes the audit
    honest. Tetragon's events carry a container id derived from the cgroup; to say "this syscall was
    the ATTACK pod's" rather than "some container's", the lesson needs the id the cluster actually
    scheduled — and the only place that exists is the pod object, which is deleted below. Read it
    here or lose it.
    ...
    while time.monotonic() < deadline:
        ...
        wsel = (
            f'{{.status.containerStatuses[?(@.name=="{container}")]' if container else "{.status.containerStatuses[0]"
        )
        reason = kubectl(
            "-n", namespace, "get", "pod", name, "-o", f"jsonpath={wsel}.state.waiting.reason}}", check=False
        )
    ...
    sel = f'{{.status.containerStatuses[?(@.name=="{container}")]' if container else "{.status.containerStatuses[0]"
    logs_argv = ["-n", namespace, "logs", name] + (["-c", container] if container else [])
    logs = kubectl(*logs_argv, check=False, timeout=120)
    reason = kubectl(
        "-n", namespace, "get", "pod", name, "-o", f"jsonpath={sel}.state.terminated.reason}}", check=False
    )
    # `containerd://<64 hex>`; the scheme is stripped so it can be compared with Tetragon's own
    # (truncated) id by prefix.
    container_id = kubectl(
        "-n", namespace, "get", "pod", name, "-o", f"jsonpath={sel}.containerID}}", check=False
    ).split("://")[-1]
    # `delete=False` is what a MULTI-CONTAINER pod needs: this leaf must also read the SENSOR
    # container's logs, and deleting the pod here would race that read away. The caller deletes.
    if delete:
        kubectl("-n", namespace, "delete", "pod", name, "--ignore-not-found", "--now", "--wait=false", check=False)
    return phase, logs, reason, container_id
