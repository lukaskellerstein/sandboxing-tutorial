...
# is paid once, and lesson 9's own box is brought up only when the loop reaches it.
#
...
# Every DISTINCT box the lessons above resolve to. Lesson 9 owns its own (the shared node cannot
# also hold a resident OpenShell gateway on 8 GB — see lessons.json), so this is a set, not one
# name, and the trap has to destroy all of it. Derived from lessons.json rather than hardcoded:
...
# The shared cluster once, up front. Lesson 9's own box is brought up by its own leaf runner when
# the loop reaches it. up.sh is idempotent, so a leaf calling it again costs a state-file check.
"${INFRA}/up.sh" "${SHARED}"
...
for lesson in "${LESSONS[@]}"; do
  ...
  echo "  ${lesson}"
  echo "================================================================="
  # up.sh per lesson, then run.sh. For 6-8 the `up` is a no-op state-file check against the cluster
  # already running; for lesson 9 it is what brings its own box into existence. Going through
